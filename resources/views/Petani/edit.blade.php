...
@section('header', 'Edit Farmer')
@section('description', 'Update farmer partner information')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-orange-50 border-b border-orange-200">
            ...
            <p class="text-sm text-orange-700 mt-1">Update information for {{ $petani->Nama_Petani }}</p>
        </div>

        <form action="{{ route('petani.update', $petani->ID_Petani) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                @if($petani->Gambar && Storage::disk('public')->exists($petani->Gambar))
                    <div id="currentPhoto">
                        ...
                        <div class="relative inline-block">
                            ...
                            <button type="button" id="changePhoto" class="absolute -top-2 -right-2 bg-orange-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-orange-600">
                                <i class="fas fa-edit"></i>
                            </button>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Click the edit icon to change the photo</p>
                    </div>
                @endif
                ...
                <!-- Farmer Photo -->
                <div id="photoUpload" class="{{ $petani->Gambar && Storage::disk('public')->exists($petani->Gambar) ? 'hidden' : '' }}">
                    <label for="Gambar" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $petani->Gambar ? 'Change Farmer Photo' : 'Upload Farmer Photo' }}
                    </label>
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-green-400 transition-colors" id="dropZone">
                        <div class="space-y-1 text-center">
                            ...
                            <p class="text-xs text-gray-400">Leave blank if you don't want to change the photo</p>
                        </div>
                    </div>
                    @error('Gambar')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
